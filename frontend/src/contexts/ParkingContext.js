...
export const ParkingProvider = ({ children }) => {
  ...
  const generateMockParkingData = () => {
    ...
    for (let i = 0; i < 15; i++) {
      const transactionDate = new Date(
        currentDate.getTime() - Math.random() * 45 * 24 * 60 * 60 * 1000
      );
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const exitTime = new Date(
        entryTime.getTime() + (Math.random() * 8 + 1) * 60 * 60 * 1000
      );
    ...
    return {
      ...
      history: mockHistory.sort(
        (a, b) => new Date(b.entryTime) - new Date(a.entryTime)
      ),
      paymentHistory: mockPaymentHistory.sort(
  ...
  const addPaymentDeduction = async (
    amount,
    description,
    relatedSession = null
  ) => {
  ...
  const getCurrentParking = () => {
    ...
    const currentSession = parkingData.history?.find(
  ...
  const getAllParkingSlots = async () => {
    ...
    try {
      ...
      // Optional: convert status strings to lower case here
