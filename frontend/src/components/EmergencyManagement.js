...
const EmergencyManagement = () => {
  ...
  const loadEmergencySystemsData = () => {
    ...
    if (savedBollards) {
      ...
      const defaultBollards = [
        { id: 'A-1-MAIN', area: 'A', level: 1, location: 'Main Entry A-1', status: 'lowered', type: 'main_entry', active: true, emergencyOverride: false },
        { id: 'A-1-SEC', area: 'A', level: 1, location: 'Secondary Entry A-1', status: 'lowered', type: 'secondary', active: true, emergencyOverride: false },
        { id: 'B-1-MAIN', area: 'B', level: 1, location: 'Main Entry B-1', status: 'lowered', type: 'main_entry', active: true, emergencyOverride: false },
        { id: 'B-2-MAIN', area: 'B', level: 2, location: 'Main Entry B-2', status: 'lowered', type: 'main_entry', active: true, emergencyOverride: false },
        { id: 'B-2-SIDE', area: 'B', level: 2, location: 'Side Entry B-2', status: 'lowered', type: 'side_entry', active: true, emergencyOverride: false },
        { id: 'C-1-MAIN', area: 'C', level: 1, location: 'Main Entry C-1', status: 'lowered', type: 'main_entry', active: true, emergencyOverride: false },
        { id: 'C-1-EMRG', area: 'C', level: 1, location: 'Emergency Exit C-1', status: 'lowered', type: 'emergency_exit', active: true, emergencyOverride: false },
        { id: 'PERIMETER-N', area: 'PERIMETER', level: 0, location: 'North Perimeter', status: 'lowered', type: 'perimeter', active: true, emergencyOverride: false },
        { id: 'PERIMETER-S', area: 'PERIMETER', level: 0, location: 'South Perimeter', status: 'lowered', type: 'perimeter', active: true, emergencyOverride: false },
      ];
      setBollards(defaultBollards);
      localStorage.setItem('emergency_bollards', JSON.stringify(defaultBollards));
    }
    ...
    if (savedDoors) {
      ...
      const defaultDoors = [
        { id: 'MAIN-ENTRY', location: 'Main Car Park Entry', status: 'open', type: 'main_entry', active: true, emergencyOverride: false },
        { id: 'MAIN-EXIT', location: 'Main Car Park Exit', status: 'open', type: 'main_exit', active: true, emergencyOverride: false },
        { id: 'A-ENTRY', location: 'Area A Entry Door', status: 'open', type: 'area_entry', area: 'A', active: true, emergencyOverride: false },
        { id: 'A-EXIT', location: 'Area A Exit Door', status: 'open', type: 'area_exit', area: 'A', active: true, emergencyOverride: false },
        { id: 'B-ENTRY', location: 'Area B Entry Door', status: 'open', type: 'area_entry', area: 'B', active: true, emergencyOverride: false },
        { id: 'B-EXIT', location: 'Area B Exit Door', status: 'open', type: 'area_exit', area: 'B', active: true, emergencyOverride: false },
        { id: 'C-ENTRY', location: 'Area C Entry Door', status: 'open', type: 'area_entry', area: 'C', active: true, emergencyOverride: false },
        { id: 'C-EXIT', location: 'Area C Exit Door', status: 'open', type: 'area_exit', area: 'C', active: true, emergencyOverride: false },
        { id: 'EMERGENCY-EXIT-1', location: 'Emergency Exit Door 1', status: 'open', type: 'emergency_exit', active: true, emergencyOverride: false },
        { id: 'EMERGENCY-EXIT-2', location: 'Emergency Exit Door 2', status: 'open', type: 'emergency_exit', active: true, emergencyOverride: false },
      ];
  ...
  const saveEmergencyMode = (mode, level) => {
    ...
    localStorage.setItem('emergency_mode', JSON.stringify({ active: mode, level: level }));
    setLastUpdate(new Date());
  };

  const handleBollardToggle = async (bollardId) => {
    if (emergencyMode && emergencyLevel === 'lockdown') {
      alert('Cannot modify bollards during emergency lockdown. Please disable lockdown first.');
      return;
    }
    ...
    try {
      ...
      const updatedBollards = bollards.map(bollard => 
        bollard.id === bollardId 
          ? { ...bollard, status: bollard.status === 'raised' ? 'lowered' : 'raised' }
          : bollard
      );
  ...
  const handleDoorToggle = async (doorId) => {
    if (emergencyMode && emergencyLevel === 'lockdown') {
      alert('Cannot modify doors during emergency lockdown. Please disable lockdown first.');
      return;
    }
  ...
  const handleEmergencyLockdown = async () => {
    if (window.confirm('Are you sure you want to activate EMERGENCY LOCKDOWN? This will raise all bollards and close all doors.')) {
      setLoading(true);
      try {
        ...
        alert('Failed to activate emergency lockdown. Please contact system administrator.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleEmergencyEvacuation = async () => {
    if (window.confirm('Are you sure you want to activate EMERGENCY EVACUATION? This will open all doors and lower critical bollards.')) {
      setLoading(true);
      try {
        await new Promise(resolve => setTimeout(resolve, 2000)); // Simulate emergency protocol
        ...
        const updatedBollards = bollards.map(bollard => ({
          ...bollard,
          status: ['emergency_exit', 'main_entry'].includes(bollard.type) ? 'lowered' : bollard.status,
          emergencyOverride: ['emergency_exit', 'main_entry'].includes(bollard.type)
        }));
        ...
        alert('EMERGENCY EVACUATION ACTIVATED! All exits opened for evacuation.');
      } catch (error) {
        console.error('Error activating emergency evacuation:', error);
        alert('Failed to activate emergency evacuation. Please contact system administrator.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleDisableEmergency = async () => {
    if (window.confirm('Are you sure you want to disable emergency mode? This will restore normal operation.')) {
      setLoading(true);
      try {
        ...
        alert('Failed to disable emergency mode. Please contact system administrator.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const getEmergencyLevelColor = (level) => {
    switch (level) {
      ...
      case 'lockdown': return '#dc3545';
      case 'evacuation': return '#6f42c1';
          ...
          <div className="status-card">
            <div className="status-header">
              ...
              <div 
                ...
                style={{ backgroundColor: getEmergencyLevelColor(emergencyLevel) }}
              >
            ...
            <div className="status-details">
              <div className="status-item">
                <span>Mode:</span>
                <span className={`status-value ${emergencyMode ? 'emergency' : 'normal'}`}>
              ...
              <div className="status-item">
                ...
                <span className="status-value">{bollards.filter(b => b.status === 'raised').length}/{bollards.length}</span>
              </div>
              <div className="status-item">
                <span>Doors Closed:</span>
                <span className="status-value">{doors.filter(d => d.status === 'closed').length}/{doors.length}</span>
              </div>
            </div>
          </div>
          ...
          <div className="emergency-controls">
            ...
            <div className="control-buttons">
              <button 
                ...
                disabled={loading || (emergencyMode && emergencyLevel === 'lockdown')}
              >
                🔒 EMERGENCY LOCKDOWN
              </button>
              <button 
                ...
                disabled={loading || (emergencyMode && emergencyLevel === 'evacuation')}
              >
        ...
        <div className="bollards-section">
          ...
          {Object.entries(groupBollardsByArea()).map(([area, areaBollards]) => (
            <div key={area} className="area-section">
              <h3>📍 {area === 'PERIMETER' ? 'Perimeter Security' : `Area ${area}`}</h3>
                  ...
                  <div key={bollard.id} className="bollard-card">
                    <div className="bollard-header">
                      <div className="bollard-info">
                        <h4>{getBollardIcon(bollard.status)} {bollard.id}</h4>
                        <p className="bollard-location">{bollard.location}</p>
                        <p className="bollard-type">Type: {bollard.type.replace('_', ' ').toUpperCase()}</p>
                      </div>
                      <div 
                        className="bollard-status-indicator"
                        style={{ backgroundColor: getBollardStatusColor(bollard) }}
                      >
                    ...
                    <div className="bollard-controls">
                      <button
                        className={`bollard-btn ${bollard.status === 'raised' ? 'lower-btn' : 'raise-btn'}`}
                        onClick={() => handleBollardToggle(bollard.id)}
                        disabled={loading || bollard.emergencyOverride || !bollard.active}
                      >
                        {loading ? '⏳' : (bollard.status === 'raised' ? '⬇️ LOWER' : '⬆️ RAISE')}
                      </button>
                      
                      {bollard.emergencyOverride && (
                        <span className="override-badge">🚨 EMERGENCY OVERRIDE</span>
                      )}
                    </div>
                  </div>
                ))}
                  ...
                  <div key={door.id} className="door-card">
                    <div className="door-header">
                      <div className="door-info">
                        <h4>{getDoorIcon(door.status)} {door.id}</h4>
                        <p className="door-location">{door.location}</p>
                        <p className="door-type">Type: {door.type.replace('_', ' ').toUpperCase()}</p>
                        {door.area && <p className="door-area">Area: {door.area}</p>}
                      </div>
                      <div 
                    ...
                    <div className="door-controls">
                      <button
                        className={`door-btn ${door.status === 'open' ? 'close-btn' : 'open-btn'}`}
                        onClick={() => handleDoorToggle(door.id)}
                        disabled={loading || door.emergencyOverride || !door.active}
                      >
                        {loading ? '⏳' : (door.status === 'open' ? '🚫 CLOSE' : '🚪 OPEN')}
                      </button>
