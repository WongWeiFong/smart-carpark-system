...
const RevenueManagement = () => {
  ...
  const { getAllTransactions, updateTransaction, deleteTransaction, getRevenueStats, loading } = useParking();
  ...
  const filteredTransactions = useMemo(() => {
    let filtered = allTransactions.filter(transaction => {
      ...
      if (filters.user && !transaction.userName.toLowerCase().includes(filters.user.toLowerCase()) && 
          !transaction.userEmail.toLowerCase().includes(filters.user.toLowerCase())) {
        return false;
      }
  ...
  const handleSaveEdit = async () => {
    try {
      const updates = {
        amount: selectedTransaction.type === 'deduction' ? -parseFloat(editForm.amount) : parseFloat(editForm.amount),
          ...
          <div className="stats-grid">
            <div className="stat-card revenue-stat">
              ...
              <div className="stat-content">
                <div className="stat-number">${revenueStats.totalRevenue.toFixed(2)}</div>
            ...
            <div className="stat-card revenue-stat">
              ...
              <div className="stat-content">
                <div className="stat-number">${revenueStats.parkingRevenue.toFixed(2)}</div>
            ...
            <div className="stat-card revenue-stat">
              <div className="stat-icon">📊</div>
              <div className="stat-content">
                <div className="stat-number">{revenueStats.totalTransactions}</div>
        ...
        <div className="filters-section">
          <div className="filters-card">
            ...
            <div className="filter-actions">
              <button onClick={() => setFilters({dateFrom: '', dateTo: '', user: '', type: 'all', minAmount: '', maxAmount: ''})} className="reset-filters-btn">
                Reset Filters
              </button>
                  ...
                  <thead>
                    <tr>
                      <th onClick={() => handleSort('date')} className="sortable">
                        Date {sortConfig.key === 'date' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕️'}
                      </th>
                      <th onClick={() => handleSort('userName')} className="sortable">
                        User {sortConfig.key === 'userName' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕️'}
                      </th>
                      <th>Type</th>
                      <th onClick={() => handleSort('amount')} className="sortable">
                        Amount {sortConfig.key === 'amount' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕️'}
                      </th>
                      <th>Description</th>
                      <th>Actions</th>
                    </tr>
                      ...
                      <tr key={transaction.id} className="transaction-row">
                        ...
                        <td>
                          <div className="user-cell">
                            ...
                            <div className="user-email">{transaction.userEmail}</div>
                          </div>
                        </td>
                        <td>
                          <span className={`type-badge ${transaction.type}`}>
                            {transaction.type === 'topup' ? 'Top-up' : 'Parking Fee'}
                          </span>
                        </td>
                        ...
                        <td>
                          <div className="action-buttons">
                            <button onClick={() => handleViewDetails(transaction)} className="action-btn view-btn">👁️</button>
                            <button onClick={() => handleEdit(transaction)} className="action-btn edit-btn">✏️</button>
                            <button onClick={() => handleDelete(transaction)} className="action-btn delete-btn">🗑️</button>
                          </div>
                        </td>
                      </tr>
                    ))}
            ...
            <div className="modal-body">
              <div className="detail-grid">
                <div className="detail-item">
                  <label>ID:</label> <span>{selectedTransaction.reference}</span>
                </div>
                <div className="detail-item">
                  <label>User:</label> <span>{selectedTransaction.userName}</span>
                </div>
                <div className="detail-item">
                  <label>Amount:</label> 
                  <span style={{ color: getTypeColor(selectedTransaction.type) }}>
                    {selectedTransaction.type === 'topup' ? '+' : '-'}{formatCurrency(selectedTransaction.amount)}
                  </span>
                </div>
                <div className="detail-item">
                  <label>Date:</label> <span>{new Date(selectedTransaction.date).toLocaleString()}</span>
                </div>
                <div className="detail-item">
                  <label>Status:</label> <span>{selectedTransaction.status}</span>
                </div>
                <div className="detail-item">
                  <label>Method:</label> <span>{selectedTransaction.paymentMethod}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setEditModalOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button onClick={() => setEditModalOpen(false)} className="modal-close">×</button>
            </div>
            <div className="modal-body">
