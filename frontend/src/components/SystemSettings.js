...
const SystemSettings = () => {
  ...
  const loadMachinesData = () => {
    ...
    if (savedMachines) {
      ...
      const defaultMachines = [
        { id: 'A-1-ENTRY', type: 'entry', area: 'A', level: 1, location: 'Main Entrance A', status: 'automatic', active: true },
        { id: 'A-1-EXIT', type: 'exit', area: 'A', level: 1, location: 'Main Exit A', status: 'automatic', active: true },
        { id: 'B-1-ENTRY', type: 'entry', area: 'B', level: 1, location: 'Entrance B-1', status: 'manual', active: true },
        { id: 'B-1-EXIT', type: 'exit', area: 'B', level: 1, location: 'Exit B-1', status: 'automatic', active: false },
        { id: 'B-2-ENTRY', type: 'entry', area: 'B', level: 2, location: 'Entrance B-2', status: 'automatic', active: true },
        { id: 'B-2-EXIT', type: 'exit', area: 'B', level: 2, location: 'Exit B-2', status: 'manual', active: true },
        { id: 'C-1-ENTRY', type: 'entry', area: 'C', level: 1, location: 'Entrance C-1', status: 'automatic', active: true },
        { id: 'C-1-EXIT', type: 'exit', area: 'C', level: 1, location: 'Exit C-1', status: 'automatic', active: true },
      ];
  ...
  const loadParkingRates = () => {
    ...
    if (savedRates) {
      ...
      const defaultRates = {
        monday: { hourly: 5.00, daily: 30.00, weekly: 150.00 },
        tuesday: { hourly: 5.00, daily: 30.00, weekly: 150.00 },
        wednesday: { hourly: 5.00, daily: 30.00, weekly: 150.00 },
        thursday: { hourly: 5.00, daily: 30.00, weekly: 150.00 },
        friday: { hourly: 6.00, daily: 35.00, weekly: 175.00 },
        saturday: { hourly: 8.00, daily: 40.00, weekly: 200.00 },
        sunday: { hourly: 8.00, daily: 40.00, weekly: 200.00 }
      };
  ...
  const filterLogs = () => {
    ...
    if (filters.level !== 'all') {
      filtered = filtered.filter(log => log.level === parseInt(filters.level));
    }
    
    if (filters.number) {
      filtered = filtered.filter(log => log.number.toString().includes(filters.number));
    }
  ...
  const getMachineStatusColor = (machine) => {
    ...
    return machine.status === 'automatic' ? '#28a745' : '#ffc107';
  };
        ...
        {activeTab === 'machines' && (
          <div className="machines-section">
            <div className="section-header">
              ...
              <p>Control which entry and exit machines operate manually or automatically</p>
            </div>
                ...
                <div key={machine.id} className="machine-card">
                  <div className="machine-header">
                    <div className="machine-info">
                      ...
                      <p className="machine-details">Area {machine.area} - Level {machine.level}</p>
                    </div>
                    <div 
                      className="machine-status-indicator"
                      style={{ backgroundColor: getMachineStatusColor(machine) }}
                    >
                      {machine.active ? (machine.status === 'automatic' ? 'AUTO' : 'MANUAL') : 'OFF'}
                    </div>
                  </div>
                      ...
                      <div className="radio-group">
                        <label className="radio-label">
                          <input
                            ...
                            onChange={() => handleMachineStatusChange(machine.id, 'automatic')}
                        ...
                        <label className="radio-label">
                          <input
                            ...
                            onChange={() => handleMachineStatusChange(machine.id, 'manual')}
                            disabled={!machine.active}
                          />
                          👤 Manual
                        </label>
                    ...
                    <div className="control-group">
                      <label className="toggle-label">
                        <input
                          ...
                          onChange={(e) => handleMachineActiveChange(machine.id, e.target.checked)}
                        />
        ...
        {activeTab === 'logs' && (
          <div className="logs-section">
            ...
            <div className="logs-summary">
              ...
              <div className="summary-item">
                <span className="summary-number">{filteredLogs.filter(log => log.type === 'I').length}</span>
                <span className="summary-label">Entries</span>
              </div>
              <div className="summary-item">
                <span className="summary-number">{filteredLogs.filter(log => log.type === 'O').length}</span>
                    ...
                    <tr key={log.id}>
                      ...
                      <td className="location-id">
                        <span className="location-badge">
                          {log.area}-{log.level}-{log.number}{log.type}
                        </span>
                      </td>
                      <td className="log-type">
                        <span className={`type-badge ${log.type === 'I' ? 'entry' : 'exit'}`}>
          ...
          <div className="rates-section">
            ...
            <div className="rates-grid">
              {Object.keys(parkingRates).map(day => (
                <div key={day} className={`rate-card ${day === selectedDay ? 'selected' : ''}`}>
                      ...
                      {isEditingRates && day === selectedDay ? (
                        <div className="rate-input-group">
                          ...
                          <input
                            ...
                            onChange={(e) => handleRateChange(day, 'hourly', e.target.value)}
                          />
                        </div>
                      ) : (
                        <span className="rate-value">${parkingRates[day]?.hourly?.toFixed(2) || '0.00'}</span>
                      )}
                    </div>
                      ...
                      {isEditingRates && day === selectedDay ? (
                        <div className="rate-input-group">
                          ...
                          <input
                            ...
                            onChange={(e) => handleRateChange(day, 'daily', e.target.value)}
                          />
                        </div>
                      ) : (
                        <span className="rate-value">${parkingRates[day]?.daily?.toFixed(2) || '0.00'}</span>
                      )}
                    </div>
                      ...
                      {isEditingRates && day === selectedDay ? (
                        <div className="rate-input-group">
                          ...
                          <input
                            ...
                            onChange={(e) => handleRateChange(day, 'weekly', e.target.value)}
                          />
                        </div>
                      ) : (
                        <span className="rate-value">${parkingRates[day]?.weekly?.toFixed(2) || '0.00'}</span>
                      )}
                    </div>
                  </div>
                </div>
